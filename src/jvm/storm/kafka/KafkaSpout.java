...
public class KafkaSpout extends BaseRichSpout {
	...
	public SpoutNumOneTime spoutNumOneTime = null;
	public Map _conf = null;
	public TopologyContext _context = null;
	...
	public void open(Map conf, final TopologyContext context,
		...
		_conf = conf;
		_context = context;
		spoutNumOneTime = new SpoutNumOneTime();
	...
	public void nextTuple() {
		...
		for (int i = 0; i < managers.size(); i++) {
			...
			try {
				...
				EmitState state = managers.get(_currPartitionIndex).next(
						_collector, spoutNumOneTime);
				...
				/*
				 * if (state == EmitState.EMITTED_END) { LOG.info("reset!!");
				 * reSet(_conf, _context ); }
				 */
	...
	public void reSet(Map conf, TopologyContext context) {

		// _connections.clear();
		// _connections = new DynamicPartitionConnections(_spoutConfig,
		// KafkaUtils.makeBrokerReader(conf, _spoutConfig));
		/*for (PartitionManager pm : _coordinator.getMyManagedPartitions()) {
			pm.close();
		}*/
		// using TransactionalState like this is a hack
		/*int totalTasks = context
		...
		for (PartitionManager pm : _coordinator.getMyManagedPartitions()) {
			pm._emittedToOffset = pm._committedTo = 
					KafkaUtils.getOffset(
							_connections.register(pm._partition.host,pm._partition.partition),
							_spoutConfig.topic, pm._partition.partition, _spoutConfig.startOffsetTime);
		}
	}
